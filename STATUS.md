...
- [x] SEED-ORGANISMS: Design a few plausible seed organisms beyond the 12-instruction Seed Replicator, each with a stated survival strategy and a headless-runner check that it persists under the current defaults. Seven were derived, run, and reported in `docs/analysis/2026-08-23_seed-organisms.md` — a stationary chain (Radchain/Sunchain), a synthesis builder (Freesynth/Frontforge/Mixedforge), and a mobile forager (Squarestep/Wildstep, both 5/5 colonizers) — and all seven are in the frontend seed library with their evidence and verified regime.
...
- [x] MUTATION-CAUSE-SPLIT: `TickReport`, `EventTotals`, and `MetricsSnapshot` now carry `base_mutations` and `background_mutations` alongside `mutations`, which remains their sum. Attribution reuses the existing `bg_radiation_consumed > 0` branch in `mutate_end_of_tick_cell`, so no RNG draw stream moved (`grid=`/`packets=`/totals digests identical to 653999d; `check-rayon-parity.sh` green at 1/2/4/8 threads). API metrics schema 0.3.1, runner row schema unchanged at 0.2.0. The frontend charts the split on a dedicated Mutation Rates chart. See `docs/analysis/2026-08-13_mutation-load-and-emergence-milestones.md`.
...
- [ ] SEED-PROGRAM-QOL: Add seed-program library/import-export/disassembly tooling. Landed: a built-in ten-organism seed library grouped by family, a population composer (weights → largest-remainder counts → deterministic scatter onto distinct free cells), and a two-way assembler/disassembler mirroring `rust/src/opcode.rs` with an inline opcode reference (`frontend/src/lib/{opcodes,seedLibrary,populate}.ts`). Remaining: import/export of seed sets, and user-saved organisms alongside the built-ins.
...
- [ ] MAINTENANCE-DYADIC-FRACTION: Retire the last non-dyadic sampler. `WyRand::bernoulli(f64)` survives only for the fractional maintenance quantum `Bernoulli((q - floor(q)) × 2^-k)` at `rust/src/pass3.rs:686` (nonzero only when `maintenance_exponent != 1`). Make that term dyadic (e.g. stochastic rounding of `q` via `bernoulli_pow2` on the fraction's top bits, or charge `Binomial(round(q), 2^-k)`), then delete the f64 sampler. Draw-stream change for β ≠ 1 only; needs sampled-literal reconciliation. See `docs/BACKLOG.md`.
