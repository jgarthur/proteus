...
Context: the only hand-designed organism is the 12-instruction Seed Replicator in `docs/SPEC.md` ("Seed Replicator"), and the emergence work so far starts from it (see `docs/analysis/2026-08-13_mutation-load-and-emergence-milestones.md`). The goal is a small set of deliberately different organisms that are plausible under the current defaults — each with a stated survival strategy (e.g. a mobile forager that `move`s toward radiation, an `emit`/`listen` cooperative pair, a `synthesize`-driven builder that trades energy for mass) — and a headless-runner check per organism that it persists for a stated horizon with the default config. Document each genome with commented assembly like the Seed Replicator listing. Prior art to start from: `docs/analysis/2026-03-15_claude_seeds.md` proposed three organisms against spec v0.2 (Seed A "Lithotroph", Seed B "Autotroph" with synthesis in the copy loop, Seed C "Wanderer" with movement-based foraging) together with an energy/mass budget per cycle — treat them as design sketches, not drop-in genomes: they predate the absorb-footprint rework (absorb_count/absorb_dir, Pass 3 distribution), the dyadic k-form rates, per-quantum mutation triggers, and other spec changes since v0.2 (see `docs/SPEC-CHANGELOG.md`), so every budget and instruction sequence must be re-derived against the current `docs/SPEC.md`; `legacy/python/src/proteus/seed_presets.py` holds the older `basic`/`mover`/`gated`/`gated_mover` presets. Pairs naturally with SEED-PROGRAM-QOL (library/disassembly tooling) and SEED1-TRACE.
...
### MAINTENANCE-DYADIC-FRACTION: Retire the last non-dyadic sampler

Context: after DYADIC-SAMPLERS and MUTATION-ANY-QUANTUM, every probability draw in the engine is dyadic except one — the fractional maintenance quantum. `q = size^beta` is non-integer when `maintenance_exponent != 1`, and the spec charges `Binomial(floor(q), M) + Bernoulli((q - floor(q)) × M)` so the expectation stays exactly `q × M`. The Bernoulli's probability is not a power of two, so `rust/src/pass3.rs` keeps an f64 `WyRand::bernoulli` for it (the plan at `docs/analysis/2026-08-13_dyadic-sampler-plan.md` chose this deliberately). That f64 sampler now has no other production caller, so it exists only to serve this term. Joey flagged the two-sampler situation during the dyadic work; this item records the remaining half.

Options, both stream-changing only when `beta != 1` (at `beta = 1` the fractional branch is probability 0 and draws nothing, so parity must hold byte-for-byte there):
1. Stochastic rounding of `q`: draw `bernoulli_pow2` against the top bits of the fraction (e.g. compare a 32-bit uniform to `frac × 2^32`) to decide whether the extra quantum exists, then fold it into the single `binomial_pow2(n, k)` call. Expectation stays `q × M` exactly; one fewer sampler family.
2. Deterministic rounding: `Binomial(round(q), 2^-k)`. Simplest, but the expectation steps at half-integers of `q`, which biases small programs when `beta` is near 1.
Either way: delete `WyRand::bernoulli`, update SPEC.md's maintenance step and the parameter-table note about the non-dyadic fractional term, run `scripts/check-rayon-parity.sh` and a `beta = 1` twin-stream test to prove the default path is untouched, and reconcile sampled literals for the `beta != 1` fixtures.
References: `rust/src/pass3.rs` (`resolve_maintenance_cell`), `rust/src/random.rs`, `docs/SPEC.md` (Pass 3 step 9, parameter table), `docs/analysis/2026-08-13_dyadic-sampler-plan.md`
